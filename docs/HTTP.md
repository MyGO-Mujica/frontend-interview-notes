...
2. 管道化（Pipelining）
- 允许客户端连续发送多个请求，不用等上一个响应回来再发下一个
- 但服务端必须按照请求顺序依次返回响应，假设第一个请求处理很慢，后面的响应即使已经处理好了也得排队等着，还是有队头阻塞问题
- 举个例子：发了三个请求 A、B、C，B 和 C 都处理好了，但 A 还没好，B 和 C 只能干等着，无法先返回
- 所以管道化治标不治本，实际上各大浏览器也默认没有开启这个特性

3. 新增缓存控制

- 引入 Cache-Control、ETag、If-None-Match 等更灵活的缓存策略
- HTTP 1.0 只有 Expires
...
- 一个 TCP 连接上可以同时并行发多个请求，每个请求是独立的 Stream
- 彻底解决了 HTTP 1.1 的队头阻塞

2. 头部压缩（HPACK）

- HTTP 1.1 每次请求都要带完整的 Header，浪费带宽
- HTTP 2.0 用 HPACK 算法压缩 Header，相同的字段只传一次

3. 二进制分帧

- HTTP 1.1 是纯文本传输
- HTTP 2.0 把数据拆成二进制帧传输，解析更高效

4. 服务器推送（Server Push）

- 服务端可以主动推送资源，不用等客户端请求
