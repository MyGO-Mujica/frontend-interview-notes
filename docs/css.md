...
1.  px（像素）
- 类型：绝对单位
- 规则：固定像素值，不受父元素、根元素字体影响
- 特点：精准固定，兼容性最好；无法自适应，移动端适配差
- 场景：边框、小图标、固定尺寸元素

2.  em
- 类型：相对单位
- 规则：相对于父元素的 font-size
- 例：父字体 16px，1em = 16px
- 缺点：多层嵌套会层层叠加，样式容易混乱，难以管控
- 场景：文本行高、简单文字间距，现在很少用来做布局

3. rem（root em）
- 类型：相对单位
- 规则：相对于根元素 html 的 font-size，和父元素无关
- 例：html {font-size:16px}，1rem = 16px
- 优点：全局统一、可控性强，完美适配移动端 / 响应式
- 场景：移动端整体布局、宽高、间距全局适配
