...
--------
2. 转换 / 过滤类（返回新数组，不改变原数组）

- `map()`
  特点：遍历每一项，回调返回新值，最终组成新数组返回，长度和原数组一致
...
- 基本类型（number、string...）：存的是值本身
- 引用类型（对象、数组）：存的是内存地址

1. 浅拷贝
只复制第一层，嵌套的对象还是共享同一个引用

2. 深拷贝
完全独立的新对象，嵌套层级也全部复制，互不影响

##### 4. 有哪些场景可能会用到前端浅拷贝，哪些场景能够用到深拷贝？

浅拷贝：

- 更新状态：setState({ ...state, name: 'Tom' })

```js
const [user, setUser] = useState({ name: 'Tom', age: 18 })

// ❌ 直接修改，React 检测不到变化，不会重渲染
user.name = 'Jack'
setUser(user)

// ✅ 浅拷贝一个新对象，只改 name，其他字段保留
setUser({ ...user, name: 'Jack' })
```

深拷贝：

- 编辑表单时拷贝原数据，取消编辑不影响原数据
- 撤销重做功能，保存每次操作的完整快照
...
##### 2. 多次使用 bind 方法去绑定 this，它最终绑定的是哪个 this 呢？原理是什么？

>   多次调用 bind 绑定的还是第一个对象。因为 bind 的实现原理是通过闭包将 this 硬绑定到目标对象上，返回的新函数内部始终用 `apply` 将 this 指向第一次绑定的对象。后续再对返回函数调用 bind 时，虽然也会返回新函数，但新函数内部调用的是已经绑定了 this 的闭包函数，闭包中的 this 已经被固化，无法再被改变。


##### 3. 关于 this 与包装对象，以下输出多少
```
function foo() {
  console.log(this);
}
...
// 非严格模式           → Number {3}  对象
```
**严格模式（'use strict'）最核心的改变是消除了 this 的默认全局绑定**
...
什么是原型？
核心概念
每个函数天生自带一个 `prototype` 属性（叫「显式原型」），它是一个普通对象，用来存放该构造函数所有实例共享的属性和方法。每个对象（包括函数、实例）天生自带一个 __proto__ 属性（叫「隐式原型」，标准写法是 `Object.getPrototypeOf()`），它指向该对象的构造函数的 prototype。
...
###### const声明对象可以对对象里面的属性复制吗
const 声明的 "常量"，本质是变量指向的内存地址不可改变，而非值本身不可改变：
对于基本类型（数字、字符串、布尔值）：值存在栈内存中，const 声明后值无法修改。
对于引用类型（对象、数组、函数）：const 仅保证栈内存中的指针地址不变，但堆内存中对象的属性、数组的元素完全可以修改、添加、删除。

> const 本质是保证变量指向的内存地址不变，不是值不变。对象是引用类型，所以对象内部的属性完全可以赋值、修改，也可以把属性值复制给其他变量、或者复制到别的对象里，这些都不改变对象本身的引用，都是合法的；只有给这个 const 变量重新赋值一个全新的对象，才会触发报错
...
什么是词法作用域?
核心定义
**作用域在代码编写（词法分析）阶段就完全确定，和函数的调用位置无关，仅和函数声明的位置有关**。
...
定义：最外层的作用域，是代码运行的默认顶级作用域。

2. 函数作用域（Function Scope）
定义：函数内部形成的独立作用域，也叫 “局部作用域”。

3. 块级作用域（Block Scope）
定义：ES6 新增，由{}包裹的代码块（`if/for/while`等语句的花括号）形成的独立作用域。

特点
- 仅`let/const`声明的变量支持块级作用域，`var`不支持；
- 块内声明的变量仅在当前代码块内可访问，解决了var变量提升、循环变量泄漏的历史问题。
...
##### 1. 闭包泄漏是什么？
**定义本质**：闭包持有的外部变量无法被垃圾回收（GC），导致内存持续占用的泄漏问题。
**根本原因**：闭包强制持有外部变量引用，只要闭包不被销毁，变量永远无法被 GC 标记清除。
**高频场景**：全局变量持有闭包、事件监听器未解绑、定时器未清除、无过期机制的闭包缓存。
...
>函数式编程的核心是纯函数，相同输入永远返回相同输出，不修改外部状态，这样代码更可预测、好测试。另外强调不可变数据，不直接修改原数据而是返回新数据。还有高阶函数，就是函数可以作为参数或返回值，map、filter、reduce 都是典型的高阶函数。React 的函数组件、Hook 其实都有函数式编程的思想在里面。