...
- **开发环境：** 前端本地 Proxy 代理 (开发环境最常用)
  - 原理：前端请求发给本地同域服务，由本地服务转发请求给后端；服务器和服务器之间通信，不受浏览器同源限制。**服务器和服务器之间通信，不受浏览器同源限制**。

- **生产环境：** Nginx 反向代理
  - 用 Nginx 做统一入口，前端页面、后端接口共用同一个域名 / 端口，从根源消灭跨域。
...
>**JSONP 是一种利用 script 标签不受同源策略限制来实现跨域的方案。**
> 前端通过动态创建 script 标签，把回调函数名传给服务端，
> 服务端返回一段执行该回调函数的 JS 代码并携带数据，
> 浏览器加载后立即执行，从而拿到跨域数据。
>
...
#### 3. 什么是事件委托(事件代理)，e.currentTarget 与 e.target（事件对象） 有何区别

**一、核心知识点总结**

1. 什么是事件委托（Event Delegation）

- **定义**：利用**事件冒泡**的特性，不给子元素逐个绑定事件，而是把事件统一绑定到它们的父元素上，由父元素的处理函数统一管理所有子元素的事件。
...
6. 事件对象（event 对象）
触发任意事件时，浏览器会自动生成一个 event 对象，作为参数传给事件处理函数，里面包含这个事件的所有相关信息
